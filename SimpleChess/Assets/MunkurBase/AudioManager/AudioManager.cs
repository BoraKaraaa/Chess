...
namespace Munkur 
{
    ...
    {
        [SerializeField]
        private List<MusicSO> _musicList;
        private Dictionary<EMusic, MusicSO> _musicDict = new Dictionary<EMusic, MusicSO>();

        [SerializeField]
        private List<SoundEffectSO> _soundEffectList;
        private Dictionary<ESoundEffect, SoundEffectSO> _soundEffectDict = new Dictionary<ESoundEffect, SoundEffectSO>();
        ...
        public void Start()
        {
            ...
            {
                _musicDict.Add(_musicList[i].soundEffectName, _musicList[i]);
            }

            for (int i = 0; i < _soundEffectList.Count; i++) 
            {
                _soundEffectDict.Add(_soundEffectList[i].soundEffectName, _soundEffectList[i]);
            }
        ...
        * </summary>
        */
        public void PlaySoundEffect(ESoundEffect eSoundEffect)
        {
            if (!_soundEffectDict.ContainsKey(eSoundEffect))
            {
                HMDebug.LogWarning($"MusicSO {eSoundEffect} could not be found!");
                return;
            }
            
            SoundEffectSO soundEffectToPlay = _soundEffectDict[eSoundEffect];
            ...
            if (!isThereSpace) 
            {
                HMDebug.LogWarning("There is no more space for looping sound effects!");
            }
        }

        /// <summary>
        /// Directly play a given sound effect
        /// </summary>
        /// <param name="soundEffect"> is the sound effect to be played </param>
        public void PlaySoundEffect(SoundEffectSO soundEffect)
        {
            if (!soundEffect.isLooping)
            {
                _customSoundEffectAudioSource.PlayOneShot(soundEffect.audioClip, soundEffect.volume);
                return;
            }
            ...
            foreach (AudioSource soundEffectAudioSource in _soundEffectAudioSources) 
            {
                if (!soundEffectAudioSource.clip) 
                {
                    ...
                    soundEffectAudioSource.clip = soundEffect.audioClip;
                    soundEffectAudioSource.volume = soundEffect.volume;
                    soundEffectAudioSource.loop = soundEffect.isLooping;
            ...
            if (!isThereSpace) 
            {
                HMDebug.LogError("There is no more space for looping sound effects!");
            }
        }
        ...
        * </summary>
        */
        public void StopSoundEffect(ESoundEffect eSoundEffect) 
        {
            if (!_soundEffectDict.ContainsKey(eSoundEffect))
            {
                HMDebug.LogWarning($"MusicSO {eSoundEffect} could not be found!");
            ...
            SoundEffectSO soundEffectToStop = _soundEffectDict[eSoundEffect];
        ...
        */
        ...
        {
            if (!_musicDict.ContainsKey(eMusic))
            {
                HMDebug.LogWarning($"MusicSO {eMusic} could not be found!");
                return;
            }
            
            MusicSO music = _musicDict[eMusic];
        ...
        */
        private IEnumerator ChangeMusicVolumeCoroutine(float newVolume, float duration)
        {
            ...
            if (duration < 0)
            {
                HMDebug.Log("Duration must be positive.");
                yield break;
            }
        ...
        */
        ...
        /////// This part is for unity editor only ////////////////
        #if UNITY_EDITOR

                [Button]
                private void FetchSoundEffectSOAndMusicSO()
                {
                    // Fetch sound effect SO
                    SoundEffectSO[] soundEffects = Resources.LoadAll<SoundEffectSO>("SoundEffectSO");
                    ...
                    // Fetch music SO
                    MusicSO[] musics = Resources.LoadAll<MusicSO>("MusicSO");
